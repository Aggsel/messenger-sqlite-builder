import json, glob, sqlite3, os, re, datetime
...
def populate_db(file_path, db_name):
    con = sqlite3.connect(db_name)
    cur = con.cursor()

    json_object = path_to_dict(file_path)
...
def main():
    ...
    if(glob.glob(db_name)): #Should only be used for debugging purposes. TODO: remove.
        ...
        while(user_input != "Y" and user_input != "N"):
            user_input = input("Replace ? (Y/N) ").upper()
        if(user_input == "N"):
            print("Database was not replaced. Exiting.")
            return
        os.remove(db_name)

    initialize_database(db_name)

    file_paths = glob.glob(f"{folder_location}*.json")
